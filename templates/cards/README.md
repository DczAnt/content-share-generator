...
**5 套风格可选（A-E），每套 4 张，按选题调性选套系：**

### 套系总览

| 套系 | 风格 | 配色 | 适用选题 |
|------|------|------|----------|
| **A** | 混合 4 风格 | 杂志/白板/信息图/暖暗夜（每张不同） | 通用，视觉变化丰富 |
| **B** | 复古印刷风 | 做旧纸张 #f4ecd8 + 油墨 + 套色错位 | 怀旧/深度技术文 |
| **C** | 极简水墨风 | 白底 #fafafa + 墨色 + 朱砂红 + 楷体 | 简洁/方法论类 |
| **D** | 赛博暗夜风 | 深蓝黑 #0a0e27 + 霓虹青 + 扫描线 | 硬核/底层/终端向 |
| **E** | 杂志双色调风 | 米白 #f5f1e8 + 深绿 + 暖金 | 数据/优化/正式报告 |

### 每套 4 张模板

| 模板 | 尺寸 | 用途 | 对应文章位置 |
|------|------|------|------------|
| `wx-cover-*.html` | 900×383 | 封面/首图 | 文章顶部 |
| `wx-decision-tree-*.html` | 1080×720 | 决策树可视化 | "排查过程"末尾 |
| `wx-data-table-*.html` | 1080×600 | 数据对比表 | "验证"节末尾 |
| `wx-cta-*.html` | 1080×400 | 文末引导 | 文章置底 |

> 命名约定：`wx-<类型>-<套系>.html`，如 `wx-cover-C.html`。套系 A 不加后缀（`wx-cover.html`）。
